...
func TestScheduleCRUD(t *testing.T) {
	...
	// Read
	found, err := store.ScheduleFindByID(ctx, schedule.ID())
	require.NoError(t, err)
	require.NotNil(t, found)
	require.Equal(t, schedule.Name(), found.Name())
	...
	updated, err := store.ScheduleFindByID(ctx, schedule.ID())
	require.NoError(t, err)
	...
	foundAfterDelete, err := store.ScheduleFindByID(ctx, schedule.ID())
	require.NoError(t, err)
